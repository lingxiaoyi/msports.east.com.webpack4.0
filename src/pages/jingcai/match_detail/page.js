    ...
    const pageLogic = {
        ...
        get_match_bet: function () {
            ...
            hcCommon.getServerts().done(function (ts) {
                ...
                console.log('获取比赛方案成功')
            }).fail(function (err) {
                console.log('获取比赛方案失败')
                console.log(err)
            })
        },
        //获取下注列表
        ...
        buyRes: function (res) {
            res.message = (res.message.indexOf('{') > -1 && JSON.parse(res.message)) || res.message
 ...
 <div class="coin_bet">瓜分<span>${(function (total_corn) {
        ...
        matchInit: function (res) {
            ...
            res.data.forEach(function (data) {
                if (pageLogic.userData.matchid === data.match_data.matchid) {
                    if (data.match_data.ismatched === '-1') {
                        addMatch = 1
            ...
            if (!addMatch) {
                res.data.forEach(function (data) {
                    if (data.match_data.ismatched === '-1' && !addMatch) {
                        addMatch = 1
                        data.match_data.matchname = data.bet_list[0].matchname
                        let betList = pageLogic.betListInit(data.bet_list, data.match_data)
                        let Strhead = pageLogic.matchDataInit(data.match_data)
        ...
        initUserInfo: function () {
            if (this.userData.accid / 1) {
                ...
                pageLogic.userData.matchid = _util_.getUrlParam('matchid')
                ...
                $share_code.html(`<b>${pageLogic.userData.shareCode}</b><span>长按复制</span>`)
    ...
    JS_APP.ClientInfo(function (res) {
        console.log('客户端信息拉取成功')
        pageLogic.clientData = (res.version && res) || pageLogic.clientData
        pageLogic.clientData.qid = _util_.getUrlParam('qid') || pageLogic.clientData.qid
        pageLogic.run()
    })
    JS_APP.LogParameter(function (res) {
        console.log('登录信息拉取成功')
    ...
    $userInfo.on('click', '.icon', function () {
        if (!pageLogic.userData.accid) {
            JS_APP.ToViewLogin({})
            return false
        }
    ...
    $userInfo.on('click', '.user', function () {
        if (!pageLogic.userData.accid) {
            JS_APP.ToViewLogin({})
            return false
        }
    ...
    $share_btn.on('click', function () {
        ...
        /*        console.log('分享数据')
                console.log(pageLogic.share_data)
                console.log('分享数据')
    ...
    $body.on('click', 'a', function () {
        ...
        if ($this.hasClass('matchs')) {
            location.href = $(this).attr('href') + '&qid=qid10601'
            return false
        }
