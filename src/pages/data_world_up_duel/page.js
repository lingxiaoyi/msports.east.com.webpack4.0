import 'pages/data_world_up_duel/style.scss'
import 'public/libs/log.news.js'
...
$(() => {
    ...
    let _domain_ = DOMAIN
    let version = '1.0.6' //内页版本号  用来区分版本上线
    console.log(version)
    ...
    Detail.prototype = {
        ...
        common: function() {
            ...
            headerNavA.on('click', function() {
                ...
                if (type == 'data' && !navType) {
                    scope.matchDataRender('score')
                }
                if (type == 'match') {
                    scope.makeHtml['groupPhaseRender']('groupPhase', scope)
                }
            })
            // 赛程内 对阵  小组  淘汰切换
            ...
            dataNavA.on('click', function() {
                let type = $(this).attr('data-type')
                let isActive = $('.data-nav .show').attr('data-type')
                if (type === isActive && type !== 'score') return
                $(this).addClass('show')
                $(this).parent().siblings().find('a').removeClass('show')
            ...
            dataListsNavA.on('click', function() {
                let type = $(this).attr('data-type')
                let isActive = $('#dataListsNav .show').attr('data-type')
        ...
        matchHtml: {
            score: (data, self) => {
                ...
                dataScoreNavA.on('click', function() {
                    let type = $(this).attr('data-type')
                    let isActive = $('#dataScoreNav .show').attr('data-type')
                    if (type === isActive) return
                    $(this).addClass('show').siblings().removeClass('show')
                    groupArr[type] = groupArr[type].sort((a, b) => {
                        return b['积分'] - a['积分']
                    })
            ...
            scoreHtml: (newArr) => {
                let html = ''
                $.each(newArr, (i, v) => {
                    html += `<tr data-type="${v.title}"><td class="${i > 2 ? '' : 'first'}"><p class="ranking">${i + 1}</p><img src="${v['球队图标'].replace('http:', 'https:')}" alt=""><p class="name">${v['球队']}</p></td><td>${v['胜'] + '/' + v['平'] + '/' + v['负']}</td><td>${v['进/失球']}</td><td>${v['积分']}</td></tr>`
                })
        ...
        makeHtml: {
            againstPlan: function(type, self) {
                ...
                let homeImg = (img, flage) => {
                    if ((img.indexOf('8263870e94ec15958cc5414a2e8b234b') > -1 ) && flage) {
                ...
                html += `<div class="match-group-top2 match-group">
                    ...
                    str += `<div class="match-list">
                                            <div class="teams">
                                                <span class="icon">
                                                    <img onerror="this.src='//msports.eastday.com/h5/img/nomatch-r.png'" src="${homeImg(arr[0].visit_logoname, 1)}">
                                                    <div class="team">${arr[0].visit_team}</div>
                                                </span>
                                                <span class="icon">
                                                    <img onerror="this.src='//msports.eastday.com/h5/img/nomatch-r.png'" src="${visitImg(arr[0].home_logoname, 1)}">
                                                    <div class="team">${arr[0].home_team}</div>
                                                </span>
                                            </div>
                                            <div class="line">${arr[0].ismatched != 1 ? '-' : arr[0].visit_score + '-' + arr[0].home_score}</div>
                                            <span class="line-vertical"></span>
                                        </div><div class="match-list">
                                        <div class="teams">
                                            <span class="icon">
                                                <img onerror="this.src='//msports.eastday.com/h5/img/nomatch-r.png'" src="${homeImg(arr[1].home_logoname, 1)}">
                                                <div class="team">${arr[1].home_team}</div>
                                            </span>
                                            <span class="icon">
                                                <img onerror="this.src='//msports.eastday.com/h5/img/nomatch-r.png'" src="${visitImg(arr[1].visit_logoname, 1)}">
                                                <div class="team">${arr[1].visit_team}</div>
                                            </span>
                                        </div>
                                        <div class="line">${arr[1].ismatched != 1 ? '-' : arr[1].home_score + '-' + arr[1].visit_score}</div>
                                        <span class="line-vertical"></span>
                                    </div>`
                    // }
                    return str
                    ...
                    for (let item of arr) {
                        str += `<div class="match-list">
                                            ...
                                            <div class="teams">
                                                <span class="icon">
                                                    <img onerror="this.src='//msports.eastday.com/h5/img/nomatch-r.png'" src="${homeImg(item.home_logoname, 1)}">
                                                    <div class="team">${item.home_team.replace('51', '55')}</div>
                                                </span>
                                                <span class="icon">
                                                    <img onerror="this.src='//msports.eastday.com/h5/img/nomatch-r.png'" src="${visitImg(item.visit_logoname, 1)}">
                                                    <div class="team">${item.visit_team.replace('52', '56')}</div>
                                                </span>
                                            </div>
                                        </div>`
                    }
            ...
            groupPhase: function(type, self) {
                ...
                let matchPosition = $('.matchs').position().top
                let isPosition = $('.matchs .unstart').eq(0).length && $('.matchs .unstart').eq(0).position().top
                $('.matchs ').scrollTop(isPosition - matchPosition)
            },
            shootOut: function(type, self) {
                ...
                shootUl.html(str)
                let fontSize = $('html').css('font-size').replace('px', '') / 1
                let lineH = $('.matchs .line').eq(0).height()
                let unstartFirstTop = $('.matchs .unstart').eq(0).position().top - lineH - 0.69 * fontSize
                $('.matchs').scrollTop(unstartFirstTop)
            },
