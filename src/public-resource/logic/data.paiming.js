...
$(() => {
    ...
    $('.main').on('touchstart', function(e) {
        $('.pull-down-load-tips').removeClass('active')
        ...
        $('.pull-down-load-tips .iloading').css('transform', `rotate(${moveY}deg)`)
        if (moveY > touchY && $(this).find('ul:visible').scrollTop() === 0) {
            if (moveY - touchY > 30) {
                $insideNav.find('.down_load').addClass('show')
            }
            if (moveY - touchY > 60) {
                $('.pull-down-load-tips .txt').html('松开刷新')
            } else {
                $('.pull-down-load-tips .txt').html('下拉加载')
            }
            return false
        }
    }).on('touchend', function(e) {
        if (e.changedTouches[0].clientY - touchY > 60 && $(window).scrollTop() === 0) {
            $('.pull-down-load-tips').addClass('active')
            $('.pull-down-load-tips .txt').html('加载中...')
            getData[classType](true)
        } else {
            $insideNav.find('.down_load').removeClass('show')
        }
    })*/
    ...
    /*$('#data .team-list').on('scroll', function() {
        if (($(this).scrollTop()) / 100 > 0.9) {
            $goTop.show()
        } else {
            $goTop.hide()
